...
function add_selection(i) { // populate cart  array with objects?
    if (cart.length == 0) {
        cart.push(new Cart_Item(stock[i].stock_id, stock[i].stock_name, stock[i].stock_price, 1));
        display_all();
        return;
    } else { // check every element in the cart for increasing
        for (j = 0; j < cart.length; j++) {
            if (cart[j].stock_id == stock[i].stock_id) { // found it, increase the quantity by 1
                cart[j].quantities += 1;
                display_all();
                return;
            }
...
function listCart() {  //array of items
    var cartCopy = [];
    for (var i in cart) {     //loops throught every item in cart
        var item = cart[i];
        var itemCopy = {};
        for (var p in item) {   //loops through every property in item
            itemCopy[p] = item[p];
        }
...
function displayList() {
    document.getElementById("List").innerHTML =
        document.write(JSON.stringify(listCart()));
}
...
function display_all() {
    ...
    for (i = 0; i < stock.length; i++) {
        ...
        myTable += "<td style='width: 100px; text-align: right;'>" + stock[i].stock_price + "</td>";
    ...
    for (i = 0; i < cart.length; i++) {
        ...
        mycart += "<td style='width: 100px; text-align: right;'>" + cart[i].stock_price + "</td>";
        mycart += "<td style='width: 100px; text-align: right;'>" + cart[i].quantities + "</td>";
        mycart += "<td style='width: 100px; text-align: right;'>" + cart[i].stock_price * cart[i].quantities + "</td>";
        mycart += "<td><button onclick='subtract_selection(" + i + ")'>Subtract</button></td>";
    }
    mycart += "</table>";
    mycart += "<br/><td><button onclick='displayTotal()'>Total Amount</button></td>";

    mycart += "</table>";
    mycart += "<br/><td><button onclick='displayCount()'>Cart Count</button></td>";

    mycart += "</table>";
    mycart += "<br/><td><button onclick='displayList()'>Cart Order</button></td>";
...
//Stock items
var stock_item_1 = new Stock_Item(01, "Kit_Kat", 10.00, 1);
var stock_item_2 = new Stock_Item(02, "Juice", 12.00, 1);
var stock_item_3 = new Stock_Item(03, "Fizzy-Drink", 14.00, 1);
var stock_item_4 = new Stock_Item(04, "Lolly_pop", 5.00, 1);
var stock_item_5 = new Stock_Item(05, "White-Bread", 15.00, 1);
var stock_item_6 = new Stock_Item(06, "Mince", 80.00, 1);
var stock_item_7 = new Stock_Item(07, "Butter", 22.00, 1);
var stock_item_8 = new Stock_Item(08, "Cream", 35.00, 1);
var stock_item_9 = new Stock_Item(09, "Soup", 6.00, 1);
var stock_item_10 = new Stock_Item(10, "Macarroni", 35.00, 1);
